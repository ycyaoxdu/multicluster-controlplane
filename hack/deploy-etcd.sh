...
export ETCD_NS=${ETCD_NS:-"multicluster-etcd"}
export ETCD_SERVICE_NAME=${ETCD_SERVICE_NAME:-"multicluster-etcd"}
...
if [[ "${REUSE_CA}" != true ]]; then
    ...
    cd ${KUBE_ROOT}
    # copy ca to etcd dir
    mkdir -p ${KUBE_ROOT}/hack/deploy/etcd/etcd-cert
    cp -f ${CFSSL_DIR}/ca.pem ${KUBE_ROOT}/hack/deploy/etcd/etcd-cert/ca.pem

    # copy cert to controlplane dir
    CONTROLPLANE_ETCD_CERT=${KUBE_ROOT}/hack/deploy/controlplane/etcd-cert
...
cp hack/deploy/etcd/service.yaml  hack/deploy/etcd/service.yaml.tmp
sed -e 's,multicluster-etcd,'${ETCD_SERVICE_NAME}',' hack/deploy/etcd/service.yaml
...
mv hack/deploy/etcd/service.yaml.tmp hack/deploy/etcd/service.yaml
